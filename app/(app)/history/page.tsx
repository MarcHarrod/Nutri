  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={clear}
          className="flex items-center gap-1.5 text-xs text-[var(--muted)] hover:text-[var(--bad)] transition-colors"
        >
      ...
      <div className="flex flex-col gap-2">
        {entries.map((entry) => (
          <Link
            key={entry.id}
            href={`/result?barcode=${entry.barcode ?? ""}`}
            className="flex items-center gap-3 bg-[var(--surface)] rounded-2xl p-4 shadow-card hover:shadow-card-elevated transition-shadow"
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-[var(--muted)] truncate">
                {entry.brand ? `${entry.brand} · ` : ""}{timeAgo(entry.scanned_at)}
              </p>
            </div>
          </Link>
        ))}
