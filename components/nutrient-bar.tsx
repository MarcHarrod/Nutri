...
export function NutrientBar({ nutrientKey, nutrient, showServing }: { nutrientKey: string; nutrient: Nutrient; showServing?: boolean }) {
      ...
      <div className="flex-1 flex items-center gap-2">
        {pct != null ? (
          <div className="flex-1 h-1.5 bg-[var(--surface-2)] rounded-full overflow-hidden">
            <div className={cn("h-full rounded-full transition-all", tl ? TL_STYLES[tl].bar : "bg-[var(--muted-2)]")} style={{ width: `${pct}%` }} />
          </div>
        ) : <div className="flex-1" />}
        <span className="text-xs font-medium text-[var(--ink)] w-20 text-right tabular-nums">{formatNutrient(value, nutrient.unit)}</span>
      </div>
    </div>
  );
}
