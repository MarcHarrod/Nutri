...
export async function GET(req: NextRequest) {
  ...
  if (!a || !b) {
    return NextResponse.json({ error: "Both 'a' and 'b' barcode params are required" }, { status: 400 });
  }
  ...
  if (!prodA || !prodB) {
    return NextResponse.json(
      { error: `Product not found: ${!prodA ? a : b}` },
      { status: 404 }
    );
  }
  ...
  const result: CompareResult = {
    ...
    differences: keys.map((key) => {
      const na = prodA.nutrients[key];
      const nb = prodB.nutrients[key];
      const va = na.per100g;
      const vb = nb.per100g;
      let winner: "a" | "b" | "equal" = "equal";
      if (va != null && vb != null) {
        const lowerIsBetter = key !== "fibre" && key !== "protein";
        if (va < vb) winner = lowerIsBetter ? "a" : "b";
        else if (vb < va) winner = lowerIsBetter ? "b" : "a";
      }
      return {
        key,
        label: na.label,
        a_value: formatVal(va, na.unit),
        b_value: formatVal(vb, nb.unit),
        winner,
        note: null,
      };
    }),
