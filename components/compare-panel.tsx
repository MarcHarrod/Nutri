...
const COMPARE_KEYS: NKey[] = ["energy_kcal", "fat", "saturated_fat", "sugars", "fibre", "protein", "salt"];
const LOWER_BETTER: NKey[] = ["energy_kcal", "fat", "saturated_fat", "sugars", "salt"];

export function ComparePanel({ a, b }: { a: ProductAnalysis; b: ProductAnalysis }) {
  return (
    <div className="flex flex-col gap-3">
      <div className="grid grid-cols-[1fr_40px_1fr] gap-1 text-center">
        ...
        <div className="flex items-center justify-center"><span className="text-[10px] font-bold text-[var(--muted-2)]">VS</span></div>
      ...
      <div className="bg-[var(--surface)] rounded-2xl shadow-card divide-y divide-[var(--hairline-soft)]">
        {COMPARE_KEYS.map((key) => {
          const na = a.product.nutrients[key]; const nb = b.product.nutrients[key];
          const va = na.per100g; const vb = nb.per100g;
          const lowerBetter = LOWER_BETTER.includes(key);
          let winner: "a" | "b" | "equal" = "equal";
          if (va != null && vb != null && va !== vb) { if (lowerBetter) winner = va < vb ? "a" : "b"; else winner = va > vb ? "a" : "b"; }
          return (
            <div key={key} className="grid grid-cols-[1fr_80px_1fr] items-center px-3 py-2.5 gap-2">
              <span className={cn("text-sm tabular-nums text-right", winner === "a" ? "font-semibold text-[var(--good)]" : "text-[var(--ink)]")}>{formatNutrient(va, na.unit)}</span>
              <div className="flex flex-col items-center">
                ...
                ) : <Minus className="w-3 h-3 text-[var(--muted-2)] mt-0.5" />}
              </div>
              <span className={cn("text-sm tabular-nums", winner === "b" ? "font-semibold text-[var(--good)]" : "text-[var(--ink)]")}>{formatNutrient(vb, nb.unit)}</span>
            </div>
          );
        })}
      </div>
      <p className="text-xs text-[var(--muted-2)] text-center">All values per 100g. Green = better for that nutrient.</p>
    </div>
  );
}
