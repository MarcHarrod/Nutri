    ...
    <div className="flex flex-col gap-4 rise">
      ...
      <Card className="gap-3 flex flex-col">
        <div className="flex items-start justify-between gap-2">
          <div>
            <h1 className="text-lg font-semibold text-[var(--ink)] leading-snug">
              {product.name}
            </h1>
            {product.brand && (
      ...
      {analysis.good_to_know.length > 0 && (
        <Card className="flex flex-col gap-2.5 rise-2">
          <p className="text-xs font-semibold text-[var(--good)] uppercase tracking-wide">
            Good to know
          </p>
      ...
      {analysis.worth_watching.length > 0 && (
        <Card className="flex flex-col gap-2.5 rise-3">
          <p className="text-xs font-semibold text-[var(--warn)] uppercase tracking-wide">
            Worth watching
          </p>
          <div className="flex flex-col gap-2">
        ...
        <div className="bg-[var(--surface)] rounded-2xl shadow-card">
          ...
          >
            ...
            <span className="flex-1 text-sm font-medium text-left">
              Ingredient notes{" "}
              <span className="text-xs text-[var(--muted-2)] font-normal">
          ...
          {expanded === "allergens" && (
            <div className="px-4 pb-4 border-t border-[var(--hairline-soft)] pt-3">
              <p className="text-xs text-[var(--bad)] font-medium mb-2">
                Allergen information is derived from the label only. Always verify
                from the physical packaging — formulations change.
              </p>
              <div className="flex flex-wrap gap-1.5">
                {product.allergens.map((a, i) => (
                  <span
                    key={i}
                    className="text-xs bg-[var(--bad-tint)] text-[var(--bad)] px-2.5 py-1 rounded-full font-medium"
                  >
                    {a}
                  </span>
                ))}
