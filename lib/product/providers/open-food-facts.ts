...
const USER_AGENT =
  process.env.OFF_USER_AGENT ?? "Nutri/0.1 (contact@example.com)";
...
function mapOFFProduct(barcode: string, p: any): ProductData {
  ...
  return {
    ...
    last_updated: p.last_modified_t
  ...
  async search(query: string): Promise<ProductData[]> {
    try {
      const params = new URLSearchParams({
        ...
        page_size: "10",
        fields:
          "code,product_name,product_name_en,brands,image_front_url,nutriments,serving_quantity",
      });
      ...
      if (!res.ok) return [];
      const data = await res.json();
      return (data.products ?? []).map((p: { code: string }) =>
        mapOFFProduct(p.code, p)
      );
    } catch {
      return [];
    }
