  ...
  if (history.length < 2) {
    return (
      <EmptyState
        ...
        description="Scan at least two products to compare them side by side."
        ...
        {(["A", "B"] as const).map((slot) => {
          ...
          return (
            <div key={slot}>
              ...
              <select
                className="w-full text-xs bg-[var(--surface)] border border-[var(--hairline)] rounded-xl px-3 py-2.5 text-[var(--ink)]"
                value={current?.product.id ?? ""}
                onChange={(e) => {
                  const found = history.find((h) => h.analysis.product.id === e.target.value);
                  setter(found?.analysis ?? null);
                }}
              >
                <option value="">Select product</option>
                {history.map((h) => (
                  <option key={h.id} value={h.analysis.product.id}>
                    {h.analysis.product.name}
                  </option>
                ))}
              </select>
            </div>
